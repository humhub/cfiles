...
class FileList extends \yii\base\Widget
{
    ...
    public function run()
    {
        ...
        return $this->render('fileList', [
                    ...
                    'folder' => $this->folder,
                    'itemsSelectable' => !(Yii::$app->getModule('cfiles')->settings->get('disableZipSupport') && $this->folder->isAllPostedFiles()),
    ...
    {
        return array_key_exists('specialFolders', $this->items)  && sizeof($this->items['specialFolders']) > 0 
                || array_key_exists('folders', $this->items) && sizeof($this->items['folders']) > 0 
                || array_key_exists('files', $this->items) && sizeof($this->items['files']) > 0 
                || array_key_exists('postedFiles', $this->items) && sizeof($this->items['postedFiles']) > 0;
    }
