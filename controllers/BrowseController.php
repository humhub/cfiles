...
class BrowseController extends \humhub\modules\content\components\ContentContainerController
{
        ...
        foreach (UploadedFile::getInstancesByName('files') as $cFile) {
            ...
            if ($humhubFile->validate() && $humhubFile->save()) {
                ...
                if ($file->save()) {
                    ...
                    $humhubFile->save();
                    $files[] = array_merge($humhubFile->getInfoArray(), ['fileList' => $this->renderFileList()]);
                } else {
    ...
    public function actionEditFolder()
    {
        ...
        $folder = Folder::find()->contentContainer($this->contentContainer)->readable()->where(['cfiles_folder.id' => $id])->one();
        if ($folder === null) {
            $folder = new Folder();
    ...
    protected function renderFileList()
    {
        ...
        if ($folder === null) {
            $filesQuery->andWhere(['cfiles_file.folder_id' => 0]);
            $foldersQuery->andWhere(['cfiles_folder.parent_folder_id' => 0]);
        } else {
            $filesQuery->andWhere(['cfiles_file.folder_id' => $folder->id]);
    ...
    private function getCurrentFolder()
    {
        ...
        if ($folderId !== 0) {
            $folder = Folder::find()->contentContainer($this->contentContainer)->readable()->where(['cfiles_folder.id' => $folderId])->one();
        }
